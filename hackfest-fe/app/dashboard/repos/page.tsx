...
export default function GithubReposPage() {
  ...
  useEffect(() => {
    const fetchRepos = async () => {
      try {
        ...
        const res = await fetch("http://localhost:3001/api/github/all-repos", {
  ...
  const handleOpenModal = async (repo: Repo) => {
    ...
    try {
      ...
      const res = await fetch(
        `http://localhost:3001/api/github/repos/${repo.owner.login}/${repo.name}/branches`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      const data = await res.json();
  ...
  const handleSyncBranch = async (branchName: string) => {
    ...
    try {
      const token = await getToken();
      const res = await fetch(
        `http://localhost:3001/api/github/repos/${selectedRepo.owner.login}/${selectedRepo.name}/sync-branch`,
        {
          method: "POST",
